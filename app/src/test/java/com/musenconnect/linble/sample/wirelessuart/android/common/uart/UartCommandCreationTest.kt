...
class UartCommandCreationTest {
    ...
    fun `create_レジスタ値アクセス_取得_コマンド`() {
        assertEquals("020102", UartCommandRegisterRead(RegisterNumber(2)).toByteArray().toHexString())
    }

    @Test
    fun `create_レジスタ値アクセス_設定_コマンド`() {
        assertEquals("0302040F", UartCommandRegisterWrite(RegisterNumber(4), 0x0F).toByteArray().toHexString())
    }
    ...
    fun `create_センササンプリング実行要求_コマンド`() {
        assertEquals("020405", UartCommandSensorSampling(DurationIntervalSeconds(5)).toByteArray().toHexString())
    }
    ...
    fun `create_デバイス名_設定_コマンド`() {
        assertEquals("1A0653616D706C652D55617274436F6E74726F6C6C65722D303031", UartCommandDeviceNameWrite(AsciiString("Sample-UartController-001")).toByteArray().toHexString())
    }
}
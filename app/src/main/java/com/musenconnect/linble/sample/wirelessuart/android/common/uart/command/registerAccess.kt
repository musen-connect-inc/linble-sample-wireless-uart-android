...
class UartResponseRegisterRead(rxPayload: ByteArray): UartResponse(rxPayload) {
    ...
    override fun toString(): String {
        return "<${javaClass.simpleName}: registerNumber=$registerNumber, hexdecimal=${ "0x%02X".format(hexdecimal) }>"
    }
}


/** レジスタ値アクセス.設定.コマンド */
class UartCommandRegisterWrite(val registerNumber: RegisterNumber, val hexdecimal: Byte): UartCommand() {
